...
def main(_):
    ...
    if FLAGS.job_name == "ps":
        ...
        # class_count表示需要训练的图片的种类数，没有或者只有一种是会报错。
        class_count = len(image_lists.keys())
        if class_count == 0:
        ...
        with sv.managed_session(server.target) as sess:
            ...
            step = 0
            while not sv.should_stop() and step < FLAGS.training_steps:
                # 获取训练数据
                if do_distort_images:
                ...
                step, train_summary, _ = sess.run([global_step, merged, train_step],
                                            feed_dict={bottleneck_input: train_bottlenecks,
                                                       ground_truth_input: train_ground_truth})
                train_writer.add_summary(train_summary, step)

                validation_summary = sess.run(merged,
                                              feed_dict={bottleneck_input: validation_bottlenecks,

                                                            ground_truth_input: validation_ground_truth})
                validation_writer.add_summary(validation_summary, step)

                is_last_step = (step + 1 == FLAGS.training_steps)
                if (step % 10) == 0 or is_last_step:
                    ...
                    print('%s: Step %d: Train accuracy = %.1f%%,Cross entropy = '
                          '%f,Validation accuracy = %.1f%% (N=%d)' %
                          (datetime.now(), step,
                           train_accuracy * 100,
                           cross_entropy_value,
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--ps_hosts',
        type=str,
        default='192.168.1.105:2222',
        help="Comma-separated list of hostname:port pairs"
    )
    parser.add_argument(
        '--worker_hosts',
        type=str,
        default='192.168.1.104:2223, 192.168.1.106:2224',
        help="Comma-separated list of hostname:port pairs"
    )
    parser.add_argument(
        '--job_name',
